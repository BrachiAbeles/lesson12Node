...
router.get("/",(req,res)=>{
    gameModul.find({}).then(allGames=>{
        ...
        res.status(400).send("‹– ‡ŸÍﬂ Á—‹ –Í ‘ﬁÈ◊ÁŸ›")
    })
})
...
router.get("/:gameId",(req,res)=>{
    let game=arrGames.find(item=>item.id==req.params.gameId)
        if(!game)
        return res.statuse(404).send("‘ÍË◊È‘ È“Ÿ–‘ —Á—‹Í ‘‡Í’‡Ÿ›")
    res.json(game)   
})
...
router.delete("/:gameId",(req,res)=>{
    ...
    if(index==-1)
       return res.statuse(404).send("–Ÿﬂ €÷‘ ‰ËŸÿ ‹ﬁ◊ŸÁ‘")
...
router.put("/:gameId",(req,res)=>{
    let game=arrGames.find(item=>item.id==req.params.gameId);
    if(!game)
       return res.status(404).send("–Ÿﬂ €÷‘ ‰ËŸÿ ‹–ËŸ€‘")
